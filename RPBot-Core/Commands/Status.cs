...
namespace RPBot
{
    [Group("status"), Aliases("st"), Description("Status commands"), RequireOwner, IsMuted]
    class Status : BaseCommandModule
    {
        ...
        public async Task Create(CommandContext e, DiscordMember person1, DiscordMember person2)
        {
            ...
            {
                var status_two = statuses_two.OrderBy(x => Math.Abs((x.Datetime - status_one.Datetime).Ticks)).First();
